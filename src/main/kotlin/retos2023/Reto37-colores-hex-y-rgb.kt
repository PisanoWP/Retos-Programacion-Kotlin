...
fun main() {
    ...
    * pero la pongo aquí comentada para que este a la vista
...
fun hexToRgb(hex: String): String {
    ...
    if (regex.matches(hex)) {
        val red = hex.trim('#').substring(0, 2)
        val green = hex.trim('#').substring(2, 4)
        val blue = hex.trim('#').substring(4, 6)

        rgb = "(" + red.toInt(16).toString() + "," + green.toInt(16).toString() + "," + blue.toInt(16).toString() + ")"

    } else {
        println("ERROR $hex NO ES UN COLOR HEXADICMAL VÁLIDO PARA CONVERTIR A RGB")
...
fun rgbToHex(rgb: String): String {
    ...
    if (regex.matches(rgbDummy)) {
        rgbDummy = rgbDummy.replace('(', ' ').trim()
        rgbDummy = rgbDummy.replace(')', ' ').trim()

        val rgbArray = rgbDummy.split(',')
